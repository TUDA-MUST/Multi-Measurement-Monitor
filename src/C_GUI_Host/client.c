...
void free_client_info(ClientInfo *info){
    ...
    // Free measurement buffer (paged)
    if (info->measurement_data) {
        for (uint64_t i = 0; i < info->allocated_pages; i++) {
            if (info->measurement_data[i]) {
                g_free(info->measurement_data[i]);
                info->measurement_data[i] = NULL;
            }
        }
        g_free(info->measurement_data);
        info->measurement_data = NULL;
    }
