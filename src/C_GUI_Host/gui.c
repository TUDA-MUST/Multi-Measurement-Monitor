...
size_t floats_per_page = PAGE_SIZE_BYTES / sizeof(float);

// Get float for row i, channel j
static inline float get_measurement(ClientInfo *client, uint64_t row, int channel) {
    size_t row_floats = *client->settings->float_number;
    size_t index = (row * row_floats + channel);
    size_t page = (index / floats_per_page);
    size_t offset = index % floats_per_page;
    return client->measurement_data[page][offset];
}
...
static void on_send_btn_clicked(GtkButton *button, gpointer user_data) {
    ...
    info->measurement_data = g_malloc0(sizeof(float*) * MAX_NUM_MESUREMENT_PAGES);
    info->allocated_samples = 0;
    info->measurement_data[0] = g_malloc0(PAGE_SIZE_BYTES);
    info->allocated_pages = 1;
    info->allocated_samples += floats_per_page;
    info->write_index = 0;
...
static void on_export_csv_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (N >= 2) {
        ...
        * ------------------------------------------------------------ */
        ...
        /*
        //------------------ WRITE TIME SYNC ENTRIES INTO COLUMNS------------------ 
        if (client->client_time_sync && client->time_write_index > 0) {
            for (uint64_t i = 0; i < client->time_write_index; i++) {
                ...
                // Single-line output
                fprintf(fp,
                    "Sent Timesync request at %s.%03ld, received echo at %s.%03ld, local time %.1f\n",
                    sent_str, sent_ms,
                    recv_str, recv_ms,
                    ct->local_time);
            }
        ...
        */
        ...
        //------------------ WRITE TIME SYNC ENTRIES INTO LINES ------------------
        if (client->client_time_sync && client->time_write_index > 0) {
            char sent_str[80], recv_str[80];
            struct tm sent_tm, recv_tm;

            // Line 1: Sent timestamps
            fprintf(fp, "Sent timestamps at: ");
            for (uint64_t i = 0; i < client->time_write_index; i++) {
                ClientTime *ct = &client->client_time_sync[i];
                ...
                fprintf(fp, "%s.%03ld", sent_str, sent_ms);
                if (i != client->time_write_index - 1) fprintf(fp, ";");
            }
            fprintf(fp, "\n");

            // Line 2: Received timestamps
            fprintf(fp, "Received timestamps at: ");
            for (uint64_t i = 0; i < client->time_write_index; i++) {
                ClientTime *ct = &client->client_time_sync[i];
                ...
                fprintf(fp, "%s.%03ld", recv_str, recv_ms);
                if (i != client->time_write_index - 1) fprintf(fp, ";");
            }
            fprintf(fp, "\n");

            // Line 3: Local times
            fprintf(fp, "Local times of echo: ");
            for (uint64_t i = 0; i < client->time_write_index; i++) {
                ClientTime *ct = &client->client_time_sync[i];
                fprintf(fp, "%.1f", ct->local_time);
                if (i != client->time_write_index - 1) fprintf(fp, ";");
            }
            fprintf(fp, "\n");
        }
         ...
         * ============================================================ */
        fprintf(fp, "### ");        //regex for second header line
        ...
        for (uint64_t i = 0; i < client->write_index / *client->settings->float_number; i++) {
            ...
            double local_time = get_measurement(client, i, *client->settings->float_number - 1);
            ...
            double fractional = utc_sec - sec;
            int usec = (int)(fractional * 1000000.0);
...
static gboolean compute_autoscale_y_range(ClientInfo *info, uint64_t start, uint64_t count, double *out_ymin, double *out_ymax)
{
    ...
    for (uint64_t i = 0; i < count; i++) {
        uint64_t row = start + i;
        for (guint ch = 0; ch < num_floats - 1; ch++) { // skip last float (time channel)
            if (chan_checkbuttons &&
                !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(chan_checkbuttons[ch])))
                continue;
            double v = get_measurement(info, row, ch);
            if (v < ymin) ymin = v;
            if (v > ymax) ymax = v;
            found = TRUE;
        }
...
static gboolean on_plot_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    if (info->write_index == 0) return FALSE;
    ...
    double x_axis_y = H - 30;
    double y_axis_x = 40;

    // X-axis
    cairo_move_to(cr, y_axis_x, x_axis_y);
    cairo_line_to(cr, W - 10, x_axis_y);
    cairo_stroke(cr);

    // Y-axis
    cairo_move_to(cr, y_axis_x, 10);
    cairo_line_to(cr, y_axis_x, x_axis_y);
    cairo_stroke(cr);
    ...
    /* === TIME RANGE === */
    double t_start = get_measurement(info, start, (*info->settings->float_number)-1) / 1e6;
    double t_end   = get_measurement(info, start + count - 1, (*info->settings->float_number)-1) / 1e6;
    double t_range = t_end - t_start;
    if (t_range <= 0) t_range = 1e-9;
    ...
    for (int i = 0; i <= num_yticks; i++) {
        double mv = y_min + (i * y_range / num_yticks);
        double y = x_axis_y - ((mv - y_min) / y_range) * (x_axis_y - 10);
    ...
    for (int ch = 0; ch < (*info->settings->float_number)-1; ch++) {
        ...
        if (!fast_mode) {
            for (uint64_t i = 0; i < count; i++) {
                uint64_t row = start + i;
                float v_mV = get_measurement(info, row, ch);
                double t_sec = get_measurement(info, row, (*info->settings->float_number)-1) / 1e6;
            ...
            for (int px = 0; px < plot_width_px; px++) {
                uint64_t i0 = start + (uint64_t)px * samples_per_pixel;
                uint64_t i1 = i0 + samples_per_pixel;
                if (i1 > start + count) i1 = start + count;

                float vmin =  FLT_MAX;
                float vmax = -FLT_MAX;

                for (uint64_t row = i0; row < i1; row++) {
                    float v = get_measurement(info, row, ch);
                    if (v < vmin) vmin = v;
                    if (v > vmax) vmax = v;
                }
...
static gpointer measurement_thread_func(gpointer user_data) {
    ...
    while (info->measurement_active && !info->destroying) {
        ...
        */
        ...
        if (pkg.package_type == DATA_PACKAGE && pkg.package_size > 0) {
            size_t row_floats = *info->settings->float_number;       // number of floats per row
            size_t row_size_bytes = row_floats * sizeof(float);      // size of a row in bytes
            size_t n_rows = pkg.package_size / row_size_bytes;       // number of rows in this package

            float *src = (float *)pkg.package;

            for (size_t row = 0; row < n_rows; ++row) {
                size_t page_index = (info->write_index* row_floats) / floats_per_page;
                size_t offset_in_page = (info->write_index* row_floats) % floats_per_page;

                // Check if we need a new page
                if (page_index >= info->allocated_pages) {
                    if (page_index >= MAX_NUM_MESUREMENT_PAGES) {
                        g_warning("Reached MAX_NUM_MESUREMENT_PAGES, stopping measurement.");
                        info->measurement_active = FALSE;
                    ...
                    info->measurement_data[page_index] = g_malloc0(PAGE_SIZE_BYTES);
                    info->allocated_pages++;
                    info->allocated_samples += floats_per_page;
                }

                float *dest_page = info->measurement_data[page_index];

                // If the current row fits in the remaining space of the page
                if (offset_in_page + row_floats <= floats_per_page) {
                    memcpy(dest_page + offset_in_page, src, row_size_bytes);
                    src += row_floats;
                    info->write_index += 1;
                } else {
                    // Row splits across two pages
                    size_t first_part = floats_per_page - offset_in_page;
                    memcpy(dest_page + offset_in_page, src, first_part * sizeof(float));
                    src += first_part;
                    ...
                    if (page_index >= info->allocated_pages) {
                        if (page_index >= MAX_NUM_MESUREMENT_PAGES) {
                            g_warning("Reached MAX_NUM_MESUREMENT_PAGES while splitting row, stopping measurement.");
                            info->measurement_active = FALSE;
                    ...
                    size_t remaining = row_floats - first_part;
                    memcpy(info->measurement_data[page_index], src, remaining * sizeof(float));
                    src += remaining;
                    info->write_index += 1;
                }
            }

            // Request a redraw of the plot
            GtkWidget *plot_area = g_object_get_data(G_OBJECT(info->tab_content), "plot_area");
            if (plot_area)
                g_idle_add((GSourceFunc)gtk_widget_queue_draw, plot_area);
        }
        /*
        ...
        */
        else if (pkg.package_type == TIMESTAMP_PACKAGE && pkg.package_size >= sizeof(struct timespec) + sizeof(float)) {
            // Check if we need to expand the buffer
            if (info->time_write_index >= info->allocated_client_times) {
                // Double the buffer size 
                size_t new_size = info->allocated_client_times * 2;
                ClientTime *new_buffer = realloc(info->client_time_sync, new_size * sizeof(ClientTime));
                if (!new_buffer) {
                    g_warning("Failed to realloc ClientTime buffer. Dropping timestamp.");
                    break;  
                }
                info->client_time_sync = new_buffer;
                info->allocated_client_times = new_size;
            }

            // Safe to write to buffer now
            ClientTime *ct = &info->client_time_sync[info->time_write_index];
